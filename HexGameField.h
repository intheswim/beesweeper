...
class HexGameField : public GenericField
{
    public:

    HexGameField (int w, int h, Uint32 userEventID, SDL_Renderer *r, bool highlight_cells);
	~HexGameField ();

    // forbid coping since we have raw pointer in class.
    HexGameField(const HexGameField &) = delete;
    HexGameField& operator = (const HexGameField &) = delete;

    char gameType() const { return GAME_HEXAGONS; }

    protected:

	bool IsRealCell (int A, int B) const;
    void DrawCellOutline (CGContextRef g, int A, int B) const; // private?
	...
	bool initImages (SDL_Renderer *, bool highlight);
	void freeImages ();	

    void closedCellImage (SDL_Renderer *, bool highlight_cells); // closed cell image
    ...
    bool IsCornerSuit = false; 

    Image * openedBmp = nullptr;	
	Image * blastBmp = nullptr;
	Image * closedBmp = nullptr;

    Image * hex_bottom = nullptr;
	Image * hex_right_bottom = nullptr;
	Image * hex_right_top = nullptr;
